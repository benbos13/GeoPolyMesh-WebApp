 ...
 * @returns {{tags: Tag[], file_structure: Node}} The fully loaded configuration file.
 */
function configure(file_url) {
    ...
    checkType(data.tags, "tags", "array", false);
    checkType(data.fileStructure, "fileStructure", "object", false);

    let tags = [];
    let rootTag = "";
    for (let tag_constructor of data.tags) {
        let tag = new Tag(tag_constructor);
        tags.push(tag);
    ...
    let file_structure = new Node(data.fileStructure, tags);
